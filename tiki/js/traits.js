define([
    ...
    */
    ...
    // Treat bigints (64-bit) as strings containing only numbers with optional leading -
    Traits.BigInt = Trait.extend("Traits.BigInt", {
        constructor: function(config) {
            if (this instanceof Trait) this.initialize.apply(this, arguments); else return new Traits.BigInt(config);
        },
        parse: function(v) {
            if(v == null || v === '') {
                return null;
            ...
            v = String(v);
            var re = new RegExp(/^(-?)\d+$/); // Match any negative or positive number
            if(!re.test(v)) {
                throw new TypeError("Bigint not numerical: "+v);
            }
            return v;
        },
        toJSON: function(v) {
